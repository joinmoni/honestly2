...
import { Ban, Check, Pencil, Plus, Search, Shield } from "lucide-react";
import { AdminCreateVendorModal } from "@/components/admin/AdminCreateVendorModal";
import { AdminPagination } from "@/components/admin/AdminPagination";
import { AdminTopNav } from "@/components/admin/AdminTopNav";
import { EmptyState } from "@/components/ui/EmptyState";
import { BodyText, metaTextClassName, PageTitle } from "@/components/ui/Typography";
import { cn } from "@/lib/utils";
import { createAdminVendor, updateAdminVendorStatus } from "@/lib/admin-vendors.client";
...
export function AdminVendorDirectoryScreen({ data }: AdminVendorDirectoryScreenProps) {
  ...
  const handleCreateVendor = async (input: Parameters<typeof createAdminVendor>[0]) => {
    setCreatingVendor(true);
    setErrorMessage(null);

    try {
      const createdVendor = await createAdminVendor(input);
      setVendors((current) => [createdVendor, ...current]);
      setCreateVendorOpen(false);
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "This vendor could not be created right now.");
    } finally {
        ...
        <main className="mx-auto max-w-6xl px-6 py-12">
          <header className="mb-12 flex flex-col justify-between gap-8 md:flex-row md:items-end">
            ...
            <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row md:items-center">
              ...
              <button
                type="button"
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-stone-900 px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-white shadow-xl shadow-stone-200 transition-all hover:bg-stone-800"
                onClick={() => setCreateVendorOpen(true)}
              >
                <Plus size={14} strokeWidth={3} />
                {data.createVendorLabel}
              </button>
            </div>
          </header>
          ...
          {filteredVendors.length ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-left">
                  <thead className="border-b border-stone-100 bg-stone-50">
                    <tr>
                      <th className={cn("px-8 py-5 text-left font-normal", metaTextClassName)}>Vendor</th>
                      <th className={cn("px-6 py-5 text-left font-normal", metaTextClassName)}>Status</th>
                      <th className={cn("px-6 py-5 text-left font-normal", metaTextClassName)}>Ownership</th>
                      <th className={cn("px-6 py-5 text-left font-normal", metaTextClassName)}>Category</th>
                      <th className={cn("px-8 py-5 text-right font-normal", metaTextClassName)}>Actions</th>
                    </tr>
                  </thead>
            ...
            <EmptyState
              ...
              ctaLabel={query.trim() ? "Clear search" : data.createVendorLabel}
              onCta={query.trim() ? () => setQuery("") : () => setCreateVendorOpen(true)}
            />
          )}

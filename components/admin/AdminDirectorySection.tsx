  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-stone-200 bg-white shadow-sm xl:col-span-2">
      <div className="flex flex-col items-start justify-between gap-4 border-b border-stone-100 p-6 md:flex-row md:items-center">
        <h2 className="text-xl italic">Active Vendor Directory</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-stone-50">
            {rows.map((row) => (
              <tr key={row.id}>
                <td className="px-8 py-6 text-sm font-semibold">{row.vendorName}</td>
                ...
                <td className="px-8 py-6">
                  <span className={row.verificationLabel === "Vetted Creator" ? "text-xs font-medium italic text-stone-600" : "text-xs font-medium text-stone-400"}>

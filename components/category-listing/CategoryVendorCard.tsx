  ...
  return (
    <motion.article whileHover={{ y: -4 }} transition={{ duration: 0.2 }} className="group cursor-pointer">
      <Link href={`/vendor/${vendor.slug}`}>
        <div className="relative mb-4 aspect-[4/5] overflow-hidden rounded-[2rem] bg-stone-100 shadow-sm transition-shadow hover:shadow-xl hover:shadow-stone-200/50">
          ...
          <div className="absolute right-4 top-4">
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} type="button" className="rounded-full bg-white/90 p-2.5 shadow-sm backdrop-blur-sm" aria-label="Save vendor">
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <h3 className="text-2xl">{vendor.name}</h3>
            <div className="rounded bg-amber-50 px-2 py-0.5 text-sm font-bold text-amber-700">★ {vendor.ratingAvg.toFixed(1)}</div>
          </div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-stone-400">
            {style}
            {location ? ` • ${location.city}${location.region ? `, ${location.region}` : ""}` : ""}
          </p>
          {vendor.headline ? <p className="text-sm italic leading-relaxed text-stone-500">&quot;{vendor.headline}&quot;</p> : null}
        </div>
      </Link>
    </motion.article>
  );
